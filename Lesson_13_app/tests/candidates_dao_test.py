from Lesson_13_app.candidates.dao.candidates_dao import CandidateDAO
import pytest
...
@pytest.fixture()
def candidates_dao():
    candidates_dao_instances = CandidateDAO('../data/candidates.json')
    return candidates_dao_instances

# Задаем, какие ключи ожидаем получать у кандидата
keys_should_be = {'pk', 'name', 'position'}

class test_candidate_dao:
    ...
    def testgetbyid(self, candidates_dao):
